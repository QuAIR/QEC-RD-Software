...
- 支持两类一等入口：平台内建电路目录生成，以及外部电路导入。
- 将 repetition code、rotated surface code、unrotated surface code、toric code 纳入平台内建电路目录范围。
- 将 code 模块定位为“内建电路数据库 / 选择器”：用户通过内建关键字和参数生成电路，而自定义研究输入通过电路导入进入主干。
...
- 平台内建支持 repetition code、rotated surface code、unrotated surface code、toric code 作为生成型电路入口。
- 用户侧的通用交互以生成型电路或导入型电路为中心，而不是任意自定义 code 定义。
...
- 从 `CodeSpec` 生成内建目录电路
- 从 repetition code、rotated surface code、unrotated surface code、toric code 的结构化内建定义生成电路
- 导入并包装外部电路
- 电路基础检查
...
- `ExperimentConfig`
  鐢ㄤ簬鑱氬悎鐢熸垚/瀵煎叆鐢佃矾杈撳叆銆乶oise銆乨ecoder銆乻ampling銆乤nalysis 绛夊疄楠岃鏍笺€?
- `CodeSpec`
  用于表达内建电路选择与生成参数，例如内建 family key、distance、rounds、logical_basis、schedule variant、layout 等。
- `NoiseModel`
...
`CircuitArtifact` 不能只服务“内建电路目录生成”，还必须作为平台统一电路入口，允许外部电路导入。
...
- `CodeSpec` 扩展
  允许后续增加新的内建电路 family、schedule 参数和实验元数据，而不破坏现有主链；第一阶段至少覆盖 repetition、rotated surface、unrotated surface、toric。
- `NoiseModel` 扩展
...
- 支持 `stim.Circuit`
- 支持 `.stim` 文件
- 至少支持一个基础内建电路条目的 memory experiment
...
- 电路生成/导入
- repetition、rotated surface、unrotated surface、toric 四类内建电路生成
- 导入外部电路作为用户自定义码电路的入口
- Stim 驱动采样
- DEM 与图构建
- 基础本地解码
...
- 已支持内建目录电路与外部 Stim 电路导入两条研究路径
- 已支持 MWPM 与 BP+OSD 两类常见本地解码路径
- 已为后续扩展更多内建电路 family、噪声模型、TensorQEC 能力与工程能力留下稳定主干
