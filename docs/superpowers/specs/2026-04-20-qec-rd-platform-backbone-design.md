...
- `NoiseModel`
  用于表达现象学噪声、基础物理噪声，以及未来可扩展的 leakage、correlated noise 参数槽位。
...
- `CodeSpec` 扩展
  允许后续增加新的 code family、schedule 参数和实验元数据，而不破坏现有主链。
- `NoiseModel` 扩展
  允许后续增加 leakage、correlated noise、hardware-calibrated 参数等字段。
- DEM/图构建策略
  允许未来增加不同的 DEM 注释、过滤、边权和 graph/hypergraph 构造策略，但输出仍归一到 `DemArtifact` / `DecodingGraph`。
- 运行数据入口
